...
import { fetchUserPosts, deletePost, ForumPost } from "@/app/forum/forum-helper/helper";
import Link from "next/link";
import { Edit2, Trash2, Calendar, Clock, MessageSquare, AlertCircle, Trash } from "lucide-react";
...
const DeleteConfirmationModal = ({ isOpen, onClose, onConfirm, postTitle, isDeleting }: DeleteModalProps) => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in duration-200">
                <div className="flex flex-col items-center text-center">
                    ...
                    <h3 className="text-xl font-black text-white mb-2 tracking-tight">Delete Post?</h3>
                    <p className="text-slate-400 text-sm leading-relaxed mb-8">
                        Are you sure you want to delete <span className="text-slate-200 font-bold">"{postTitle}"</span>? This action cannot be undone and the cosmic energy will be lost forever.
                    </p>
                ...
                <div className="space-y-4 w-full">
                    {isLoading ? (
                        ...
                        <div className={`flex flex-col items-center justify-center p-16 text-center rounded-[32px] border-2 border-dashed ${isDark ? 'bg-slate-950/50 border-slate-800/50' : 'bg-slate-50/50 border-slate-200'}`}>
                            ...
                            <h3 className={`text-2xl font-black mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>You haven't posted anything yet</h3>
                            ...
                            >
                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <Link href={`/forum/${post.id}`} className={`block text-xl font-black mb-3 transition-colors ${isDark ? 'text-white hover:text-indigo-400' : 'text-slate-900 hover:text-indigo-600'}`}>
                                            {post.title}
                                        </Link>
