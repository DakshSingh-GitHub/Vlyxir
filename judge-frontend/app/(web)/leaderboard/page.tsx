...
function TopThreeCard({ user, rank, delay, color, shadow, height, isMain }: any) {
    ...
    return (
        ...
        >
            ...
            <div className="text-center space-y-1 mt-4 z-20">
                ...
                <div className="pt-2">
                    <span className="text-3xl font-black dark:text-slate-100 text-slate-900">
                        {user.total_score?.toLocaleString() || 0}
                    </span>
                    <span className="text-xs font-bold text-slate-500 ml-1 uppercase tracking-widest">pts</span>
                </div>
            </div>
            ...
            <div className={`w-full ${height} rounded-t-3xl bg-linear-to-b from-white/10 to-transparent dark:from-slate-800/40 dark:to-transparent border-x border-t border-slate-200 dark:border-slate-800 mt-4 backdrop-blur-md relative overflow-hidden flex flex-col items-center p-6`}>
                ...
                <div className="mt-4 flex flex-col items-center gap-2 relative z-10">
                    <span className={`text-[10px] font-black uppercase tracking-[0.3em] px-3 py-1 rounded-full border ${rank === 1 ? 'border-amber-500/50 text-amber-500 bg-amber-500/5' :
                            rank === 2 ? 'border-slate-400/50 text-slate-400 bg-slate-400/5' :
    ...
    return (
        ...
        >
            <td className="px-8 py-5">
                <span className={`flex items-center justify-center h-8 w-8 rounded-lg font-black text-sm ${rank <= 3 ? 'bg-indigo-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'
                    }`}>
                    {rank}
                </span>
            </td>
            ...
            <td className="px-8 py-5 text-right">
                <Link href={`/user/${user.username}`} className="p-2 rounded-full hover:bg-indigo-500/10 text-slate-400 hover:text-indigo-400 transition-all inline-block">
