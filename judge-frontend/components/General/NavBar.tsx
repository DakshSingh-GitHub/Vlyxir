...
const NavBar: React.FC<NavBarProps> = memo(({ isSidebarOpen, setIsSidebarOpen, setIsSubmissionsModalOpen, onOpenSettings }) => {
    ...
    const { user, isLoading, signOut, savedAccounts, switchAccount, removeAccount } = useAuth();
    const displayName =
                            ...
                            <div className="relative" ref={profileRef}>
                                ...
                                >
                                    <div className="relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-full border border-indigo-200 bg-indigo-100 shadow-sm transition-transform group-hover:scale-105 dark:border-indigo-800 dark:bg-indigo-900/40">
                                        {avatarUrl ? (
                                            <Image src={avatarUrl} alt="Profile" fill sizes="32px" className="object-cover" unoptimized />
                                        ) : (
                                ...
                                {isProfileOpen && (
                                    <div className="absolute right-0 mt-3 w-80 rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
