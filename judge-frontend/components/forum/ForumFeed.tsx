    ...
    useEffect(() => {
        ...
        async function loadPosts() {
            ...
            try {
                ...
                console.log("DEBUG: Fetched posts in ForumFeed:", data);
                if (isMounted) {
                    setPosts(data || []);
                }
            } catch (error) {
    ...
    return (
        <main className="flex-1 flex flex-col min-w-0 overflow-y-auto px-4 py-6 md:px-8">
            ...
            <motion.div 
                ...
                className={`hidden md:flex gap-6 mb-8 border-b ${isDark ? 'border-slate-800' : 'border-slate-200'} overflow-x-auto no-scrollbar min-h-12`}
            >
                {tabs.map((tab) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {isActive && (
                                <motion.div 
                                    layoutId="activeTab"
                                    className="absolute -bottom-px left-0 right-0 h-[2.5px] bg-indigo-500 rounded-full z-20" 
                                />
                            )}
                        </button>
                    );
            ...
            >
                {isLoading ? (
                    ...
                    <div className="space-y-4">
                        {Array.from({ length: skeletonCount }).map((_, i) => (
                            <motion.div 
                                ...
                                className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} shadow-sm`}
                            >
                    ...
                    <motion.div 
                        ...
                        className={`mt-8 flex flex-col items-center justify-center p-12 text-center rounded-3xl border border-dashed ${isDark ? 'bg-slate-900/40 border-slate-700/50' : 'bg-slate-50/50 border-slate-300'}`}
                    >
                                    ...
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        {post.channel_name === 'Questions' && (
                                            <div className="ml-auto flex items-center gap-2">
                                                {post.selected_solution_id && (
                                                    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border shadow-sm transition-all hover:scale-105 ${
                                                ...
                                                <div className={`flex items-center justify-center w-9 h-9 rounded-xl border shadow-sm transition-all hover:scale-110 ${
            ...
            <div className="md:hidden fixed bottom-6 right-6 flex flex-col items-end gap-3 z-50">
                ...
                <motion.button
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 z-50 border-2 border-white/10 ${isFabOpen
